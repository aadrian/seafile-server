                                   ...
                                   GError **error)
{
    ...
    if (passwd && passwd[0] != 0) {
        ...
        seafile_generate_magic (CURRENT_ENC_VERSION, repo_id, passwd, salt, magic);
        if (seafile_generate_random_key (passwd, CURRENT_ENC_VERSION, salt, random_key) < 0) {
            goto bad;
        }
    }
    ...
    if (passwd)
        rc = create_repo_common (mgr, repo_id, repo_name, repo_desc, owner_email,
                                 magic, random_key, salt, CURRENT_ENC_VERSION, error);
    else
        rc = create_repo_common (mgr, repo_id, repo_name, repo_desc, owner_email,
